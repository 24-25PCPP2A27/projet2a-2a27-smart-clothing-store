...
Article::Article(int IDA, QString TYPE, QString TAILLE, QString COULEUR, int QUANTITE, float PRIX_UNITAIRE, int NBR_VENTES, QString MARQUE, QByteArray IMAGE) {
    this->IDA = IDA;
    this->TYPE = TYPE;
    this->TAILLE = TAILLE;  // Add TAILLE
...
bool Article::AjouterAritcle()
{
    ...
    query.prepare("INSERT INTO ARTICLES (IDA, TYPE, TAILLE, COULEUR, QUANTITE, PRIX_UNITAIRE, NBR_VENTES, MARQUE, IMAGE) "
                  "VALUES (:IDA, :TYPE, :TAILLE, :COULEUR, :QUANTITE, :PRIX_UNITAIRE, :NBR_VENTES, :MARQUE, :IMAGE)");
...
bool Article::ModifierAritcle()
{
    ...
    QString quantite = QString::number(QUANTITE);
    QString prixu = QString::number(PRIX_UNITAIRE);
    query.prepare("UPDATE ARTICLES SET TYPE = :TYPE, TAILLE = :TAILLE, COULEUR = :COULEUR, QUANTITE = :QUANTITE, "
                  "PRIX_UNITAIRE = :PRIX_UNITAIRE, MARQUE = :MARQUE, IMAGE = :IMAGE WHERE IDA = :IDA");


    query.bindValue(":IDA", ida);
    query.bindValue(":TYPE", TYPE);
    query.bindValue(":TAILLE", TAILLE);  // Bind TAILLE
    query.bindValue(":COULEUR", COULEUR);
    query.bindValue(":QUANTITE", quantite);
    query.bindValue(":PRIX_UNITAIRE", prixu);
    query.bindValue(":MARQUE", MARQUE);
...
QSqlQueryModel* Article::AfficherArticle()
{
    ...
    model->setHeaderData(2, Qt::Horizontal, QObject::tr("Taille"));  // Add TAILLE column
    ...
    model->setHeaderData(6, Qt::Horizontal, QObject::tr("Nombre de Ventes"));
...
bool Article::ExporterPDF(const QString &filePath)
{
    ...
    htmlContent.append("<tr><th>ID</th><th>Type</th><th>Taille</th><th>Couleur</th><th>Quantité</th><th>Prix Unitaire</th><th>Marque</th></tr>");
    while (query.next()) {
        htmlContent.append("<tr>");
        htmlContent.append("<td>" + query.value("IDA").toString() + "</td>");
        htmlContent.append("<td>" + query.value("TYPE").toString() + "</td>");
        htmlContent.append("<td>" + query.value("TAILLE").toString() + "</td>");  // Include TAILLE
